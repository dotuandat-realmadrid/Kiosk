...
const getLatestByCallTime = (list) => {
  ...
  return list.reduce((latest, cur) => {
    ...
    const dateA = latest.print_date || new Date().toISOString().slice(0, 10);
    const dateB = cur.print_date    || new Date().toISOString().slice(0, 10);
...
const sortByCallTimeAsc = (list) => {
  ...
  return [...list].sort((a, b) => {
    ...
    const dateA = a.print_date || new Date().toISOString().slice(0, 10);
    const dateB = b.print_date || new Date().toISOString().slice(0, 10);
...
/**
 * Build danh sách slides từ board data (image_sliders + videos)
 * Ưu tiên: nếu is_slider_enabled → thêm ảnh, nếu is_video_enabled → thêm video
 */
const buildMediaSlides = (board) => {
  ...
  if (board.is_slider_enabled && board.image_sliders?.length > 0) {
    board.image_sliders.forEach((item) => {
      slides.push({
        type: "image",
        src: item.file_path,   // ← "/uploads/e-center-boards/sliders/..."
        alt: item.description || "Slider",
      });
    });
  }
  ...
  if (board.is_video_enabled && board.videos?.length > 0) {
    board.videos.forEach((item) => {
      slides.push({
        type: "video",
        src: item.file_path,   // ← "/uploads/e-center-boards/videos/..."
...
export default function ECenterBoard() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (board) {
          setBoardInfo(board);
          const slides = buildMediaSlides(board);
  ...
  // ─── FETCH danh sách serving ─────────────────────────────────────────────────
  const fetchServingList = useCallback(async () => {
    try {
      const result = await searchTransaction({ status: "serving" }, 1, MAX_ROWS);
      if (result?.success) {
        const sorted = sortByCallTimeAsc((result.data || []).slice(0, MAX_ROWS));
  ...
  const handleEvent = useCallback(async (data) => {
    const list = await fetchServingList();

    if (data?.status === "serving" && data?.id) {
      const detail = await fetchDetail(data.id);
      setLatestCalled(detail || list.find((r) => r.id === data.id) || null);
      triggerFlash();
      return;
    }
  ...
  // ─── WEBSOCKET ──────────────────────────────────────────────────────────────
  UseWebSocket({
    "status-changed":     useCallback((data) => handleEvent(data), [handleEvent]),
    "update-transaction": useCallback((data) => handleEvent(data), [handleEvent]),
  ...
  useEffect(() => {
    ...
    const t = setInterval(
      () => setCurrentSlide((p) => (p + 1) % mediaSlides.length),
      5000
    );
    return () => clearInterval(t);
  }, [mediaSlides]);
      ...
      <style>{`
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .qa-logo {
          ...
          background: radial-gradient(circle, #FFD700 30%, #E65100 100%);
          display: flex; align-items: center; justify-content: center;
          font-size: 34px; box-shadow: 0 0 16px rgba(255,215,0,0.6);
          flex-shrink: 0; overflow: hidden;
        }
        .qa-logo img { width: 100%; height: 100%; object-fit: cover; border-radius: 50%; }
        .qa-title { text-align: center; flex: 1; padding: 0 14px; }
        .qa-title-top {
          font-family: 'Oswald', sans-serif; color: #FFD700;
          font-size: clamp(1rem, 2.2vw, 1.6rem); font-weight: 500;
          letter-spacing: 1.5px; text-transform: uppercase;
          text-shadow: 1px 2px 6px rgba(0,0,0,0.5); line-height: 1.25;
        }
        .qa-title-sub {
          font-family: 'Oswald', sans-serif; color: #FFD700;
        ...
        .qa-serving-badge-time {
          font-family: 'Oswald', sans-serif; color: rgba(255,215,0,0.6);
        ...
        .qa-body {
          flex: 1; min-height: 0;
          display: flex; gap: 54px; margin: 8px 42px;
        }

        /* LEFT */
        .qa-left {
          width: 32%; flex-shrink: 0;
          background: linear-gradient(170deg, #5C0000, #380000);
          border: 2px solid rgba(255,215,0,0.45); border-radius: 8px;
          padding: 14px; display: flex; flex-direction: column;
        }
        .qa-list-title {
          font-family: 'Oswald', sans-serif; color: #FFD700;
          font-size: clamp(0.85rem, 1.7vw, 1.15rem); font-weight: 700;
          text-align: center; letter-spacing: 2px; text-transform: uppercase;
          text-shadow: 0 0 10px rgba(255,215,0,0.4); margin-bottom: 4px;
        }
        .qa-list-deco {
          display: flex; align-items: center; justify-content: center;
          gap: 4px; margin: 8px 0; opacity: 0.8;
        }
        .qa-col-headers {
          display: grid; grid-template-columns: 120px 16px 1fr;
          gap: 0; padding: 0 2px; margin-bottom: 8px;
        }
        .qa-col-headers span {
          font-family: 'Oswald', sans-serif; color: #FFD700;
          font-size: clamp(0.75rem, 1.4vw, 0.95rem); font-weight: 600;
        ...
        .qa-counter {
          ...
          font-family: 'Oswald', sans-serif; font-size: clamp(1rem, 2vw, 1.35rem); font-weight: 700;
          height: 44px; display: flex; align-items: center; justify-content: center;
          border-radius: 6px 0 0 6px; border: 1px solid #42A5F5; border-right: none;
        }
        ...
        .qa-ticket {
          height: 44px; background: rgba(255,255,255,0.09);
          border: 1px solid rgba(255,215,0,0.2); border-left: none; border-radius: 0 6px 6px 0;
          display: flex; align-items: center; justify-content: center;
          font-family: 'Oswald', sans-serif; font-size: clamp(1rem, 2vw, 1.35rem); font-weight: 600; color: #FFD700;
        }
        ...
        .qa-media {
          ...
          border: 3px solid #FFD700; border-radius: 8px; overflow: hidden; background: #000;
        }
        ...
        .qa-media-empty {
          ...
          display: flex; align-items: center; justify-content: center;
          color: rgba(255,215,0,0.3); font-family: 'Oswald', sans-serif;
        ...
        /* BANNER */
        .qa-calling {
          background: linear-gradient(90deg, #7A0000, #B00000, #7A0000);
        ...
        @keyframes flash-anim {
          0%,100% { background: linear-gradient(90deg, #7A0000, #B00000, #7A0000); box-shadow: 0 0 8px rgba(255,215,0,0.3); }
          50%      { background: linear-gradient(90deg, #AA0000, #EE1100, #AA0000); box-shadow: 0 0 40px rgba(255,215,0,1); }
        }
        .qa-call-label {
          font-family: 'Oswald', sans-serif; color: #FFD700;
          font-size: clamp(1rem, 2.2vw, 1.45rem); font-weight: 400; letter-spacing: 1px;
        }
        .qa-call-ticket {
        ...
        @keyframes ticket-pop {
          0%   { transform: scale(1); }
          50%  { transform: scale(1.22); color: #FFD700; }
        ...
        .qa-footer {
          ...
          display: flex; justify-content: space-between; padding: 0 4px 2px;
        }
      `}</style>
      ...
      <div className="qa-wrap">
        ...
        <header className="qa-header">
          ...
          <div className="qa-title">
            <div className="qa-title-top">
              {"BỘ PHẬN TIẾP NHẬN TRẢ KẾT QUẢ " + boardInfo?.name || "BỘ PHẬN TIẾP NHẬN TRẢ KẾT QUẢ XÃ"}
            </div>
            <div className="qa-title-sub">Hành chính phục vụ</div>
          </div>
          <div className="qa-header-right">
            {servingTime ? (
              <div className="qa-serving-badge-time">{servingTime}</div>
            ) : (
        ...
        <div className="qa-body">
          ...
          <div className="qa-left">
            ...
            <div className="qa-col-headers">
              <span>Quầy</span><span /><span>Số vé</span>
            </div>
                    ...
                    <div key={i} className={`qa-slide${i === currentSlide ? " active" : ""}`}>
                      {slide.type === "image" ? (
                        ...
                        <video
                          ...
                          muted
                          loop
                          // Khi video kết thúc, chuyển slide tiếp theo
                          onEnded={() => setCurrentSlide((p) => (p + 1) % mediaSlides.length)}
                        />
                      )}
                    </div>
                  ))}
                  {mediaSlides.length > 1 && (
                    <div className="qa-dots">
                      {mediaSlides.map((_, i) => (
                        <div
                          key={i}
                          className={`qa-dot${i === currentSlide ? " active" : ""}`}
        ...
        <footer className="qa-footer">
          ...
          <span>{new Date().toLocaleDateString("vi-VN")} {formatTime(currentTime)}&nbsp;·&nbsp;beta-v2.0.7</span>
        </footer>
      </div>
    </>
  );
