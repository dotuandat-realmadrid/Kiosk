...
const AdminLayout = () => {
  ...
  const items = [
    ...(hasPermission(["ADMIN", "USER"])
      ...
      : []),
    ...(hasPermission(["ADMIN", "USER"])
      ? [getItem("THỐNG KÊ/BÁO CÁO", "reports-manager", <BarChartOutlined />, 0, 
          [
            getItem("Giao dịch", "/admin/transactions", <HistoryOutlined />), //SwapOutlined
          ]
      )]
      : []),
    ...(hasPermission(["ADMIN", "USER"])
      ? [
          getItem(
            ...
            0,
            [
              getItem("Định dạng vé", "/admin/ticket-formats", <TagsOutlined />),
              getItem("Dịch vụ", "/admin/services", <ShoppingCartOutlined />),
              getItem("Nhóm dịch vụ", "/admin/service-groups", <InboxOutlined />),
            ]
          ),
        ]
      : []),
    ...(hasPermission(["ADMIN", "USER"])
      ? [
          getItem("QUẢN LÝ PGD", "branches-manager", <ShopOutlined />, 0, [
            getItem("Tỉnh/Thành phố", "/admin/provinces", <GlobalOutlined />),
            getItem("Quận/Huyện", "/admin/districts", <EnvironmentOutlined />),
            getItem("Phòng giao dịch", "/admin/transaction-offices", <BankOutlined />),
            getItem("Cấu hình PGD", "/admin/branches", <SettingOutlined />),
          ]),
        ]
      : []),
    ...(hasPermission(["ADMIN", "USER"])
      ...
      : []),
    // ...(hasPermission(["ADMIN", "USER"])
    //   ? [getItem("QUẢN LÝ ĐẶT HẸN", "/admin/appointments", <CalendarOutlined />)]
    //   : []),
    ...(hasPermission(["ADMIN", "USER"])
      ? [
          getItem("NGƯỜI DÙNG", "users-manager", <TeamOutlined />, 0, [
            getItem("Tài khoản", "/admin/users", <UserOutlined />),
          ]),
        ]
      : []),
    ...(hasPermission(["ADMIN", "USER"])
      ? [getItem("ABOUT", "/admin/info", <InfoCircleOutlined />)]
      : []),
  ];
